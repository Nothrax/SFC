...
{
    std::cout << "Spoustim simulaci\nNastaveno kroku: " << swarmOptions.numberOfSteps
              << "\nPocet castic:" << swarmOptions.numberOfParticles
...
void MainWindow::handleFunctionChange(const QString& command_text) {
    ...
    delete swarmOptions.function;
    if(command_text.contains("Ackley")){
        swarmOptions.function = new Ackley();
    }else if(command_text.contains("Cross-in-tray")){
        swarmOptions.function = new CrossInTray();
    }else if(command_text.contains("Griewank")){
        swarmOptions.function = new Griewank();
    }else if(command_text.contains("Sphere")){
        swarmOptions.function = new Sphere();
    }else if(command_text.contains("Six-Hump Camel")){
        swarmOptions.function = new SixHumpCamel();
    }else if(command_text.contains("Easom")){
...
void MainWindow::repaintGraph() {
    colorMap->data()->setSize(graphResolutionX, graphResolutionY); // we want the color map to have graphResolutionX * graphResolutionY data points
    colorMap->data()->setRange(QCPRange(-swarmOptions.boundary.x, swarmOptions.boundary.x), QCPRange(-swarmOptions.boundary.y, swarmOptions.boundary.y)); // and span the coordinate range -4..4 in both key (x) and value (y) dimensions
